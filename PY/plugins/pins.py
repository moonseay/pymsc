...
@PY.on(events.NewMessage(pattern="^[?!/]pinned"))
async def get_pinned(event):
    if Config.MANAGEMENT_MODE == "ENABLE":
        return
    chat_id = (str(event.chat_id)).replace("-100", "")

    Ok = await PY.get_messages(event.chat_id, ids=types.InputMessagePinned()) 
...
@PY.on(events.NewMessage(pattern="^[!?/]pin ?(.*)"))
@is_admin
async def pin(event, perm):
    ...
    if "notify" in input_str:
       await PY.pin_message(event.chat_id, msg, notify=True)
       return
    await PY.pin_message(event.chat_id, msg)   

@PY.on(events.NewMessage(pattern="^[!?/]unpin ?(.*)"))
@is_admin
...
@PY.on(events.NewMessage(pattern="^[!?/]permapin"))
@is_admin
async def permapin(event, perm):
    ...
    if not msg:
       ...
       return
    hn = await PY.send_message(event.chat_id, msg.message)
    await PY.pin_message(event.chat_id, hn, notify=True)


@PY.on(events.NewMessage(pattern="^[!?/]unpinall"))
...
"""
    ...
    await PY.send_message(event.chat_id, UNPINALL, buttons=[
    [Button.inline("Confirm", data="unpin")], 
    [Button.inline("Cancel", data="cancel")]])

@PY.on(events.callbackquery.CallbackQuery(data="unpin"))
async def confirm(event):
    if Config.MANAGEMENT_MODE == "ENABLE":
        return
    check = await event.client.get_permissions(event.chat_id, event.sender_id)
    if check.is_creator:
        await PY.unpin_message(event.chat_id)
        await event.edit("Unpinned All Msgs!")
        return 

    await event.answer("Group Creator Required!")

@PY.on(events.callbackquery.CallbackQuery(data="cancel"))
...
@PY.on(events.callbackquery.CallbackQuery(data="pins"))
async def _(event):
